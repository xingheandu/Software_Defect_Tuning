...
# need to implement a ring model for process to communicate with each other
...
def de_sequence(fobj, bounds, mut=0.8, crossp=0.9, popsize=60, its=1000):
...
def de_island(fobj, bounds, mut=0.8, crossp=0.9, popsize=60, its=1000):
    dimensions = len(bounds)
    pop = np.random.rand(popsize, dimensions)
    min_b, max_b = np.asarray(bounds).T
    diff = np.fabs(min_b - max_b)
